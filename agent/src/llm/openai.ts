...
Analyze the given Solidity source and return ONLY a JSON object of exactly this shape:
...
Be precise and cite the most relevant line number. If you find no issues, return a single "info" finding.
Output JSON only — no prose, no markdown fences.`;

export async function auditWithOpenAI(source: string): Promise<Finding[]> {
  const client = new OpenAI({ apiKey: config.OPENAI_API_KEY, timeout: 30_000, maxRetries: 1 });
