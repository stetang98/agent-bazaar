...
"Notice. MetaMask asks for a signature, not a transaction. The buyer pays no gas. They just authorize a U S D C pull. Under the hood it's E I P three thousand and nine. Our self-hosted x four oh two facilitator submits it, and the escrow contract pulls the funds atomically, and emits an on-chain receipt."
...
"MetaMask asks for a SIGNATURE, not a tx — gasless. EIP-3009: our x402 facilitator settles atomically into PaymentEscrow."
